...
class Particle:
    ...
    def __init__(self, P_xs, P_ys, charge, mass, radius):
    ...
    def earth(self):
        global universals
        field_r, field_θ = universals["field_r"].value, universals["field_θ"].value
    ...
    def draw_mass(self):
        ...
        r_p, _ = mechanical.to_pixel(self.r_s, 0)

        pygame.draw.circle(self.colour, (P_xp, P_yp), r_p)
        if self.fixed:
            pygame.draw.circle(self.dark_colour, (P_xp, P_yp), round(r_p / 2))
    ...
    def draw_vector(self, dt=1, mouse=False):
            ...
            pygame.draw.line(self.dark_colour, (P_x1p, P_y1p), (P_x2p, P_y2p), r_p)
